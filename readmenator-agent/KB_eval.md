...
## eval/analyze.py
- Layer: utility
- Doc: Aggregate HumanEval result JSONL files into a summary table.  Reads one or more .jsonl files produced by harness.py and 
- Language: py
- Symbols:
...
- Layer: utility
- Doc: Analyze a HumanEval JSONL produced by harness.py.  For each failed problem the report shows: - the prompt fed to the mod
- Language: py
- Symbols:
...
- Doc: Diagnostico estatico de un checkpoint TopoGPT3 congelado.  Calcula sobre los pesos espectrales congelados (sin reentrena
- Language: py
- Symbols:
...
## eval/governor.py
- Layer: utility
- Doc: Streaming + governance for autoregressive generation.  Two classes that fix two real problems with the existing `topogpt
- Language: py
- Symbols:
...
- Layer: utility
- Doc: Smoke test for eval.governor (TokenStream + GenerationGovernor).  Verifies: - TokenStream threadsafety with a producer/c
- Language: py
- Symbols:
...
## eval/harness.py
- Layer: utility
- Doc: Harness for evaluating TopoGPT3 on HumanEval (164 problems).  Faithful to the official HumanEval protocol: for each prob
- Language: py
- Symbols:
...
- Layer: utility
- Doc: End-to-end smoke test of all P0+P1 components working together.  Verifies that `run_one_test_sandboxed` can run a *valid
- Language: py
- Symbols:
...
- Layer: utility
- Doc: Analisis post-hoc del noise sweep.  Generaciones del MISMO prompt bajo distintos niveles de ruido -> comparar con metric
- Language: py
- Symbols:
...
## eval/noise_sweep.py
- Layer: utility
- Doc: Barrido de ruido en los pesos espectrales del checkpoint TopoGPT3.  Para cada nivel sigma en --sigmas: 1. Carga el check
- Language: py
- Symbols:
...
## eval/repair.py
- Layer: utility
- Doc: Self-repair loop on top of a greedy JSONL.  Takes the failed problems from --input, builds a rejection-feedback prompt t
- Language: py
- Symbols:
...
## eval/report.py
- Layer: utility
- Doc: Aggregate every JSONL in eval/runs into a final report.  Reads runs from the original pass@k runs, the HRM run, the repa
- Language: py
- Symbols:
...
## eval/samplers.py
- Layer: utility
- Doc: Registry of sampler constructors for the HumanEval harness.  Replaces the hardcoded `if mode == "standard": ... elif mod
- Language: py
- Symbols:
...
## eval/sandbox.py
- Layer: utility
- Doc: Sandbox for executing model-generated code during HumanEval evaluation.  This module replaces the bare `exec()` call in 
- Language: py
- Symbols:
...
- Layer: utility
- Doc: Smoke test for eval.sandbox.  Verifies all four defence layers: L1 (AST pre-check): blocked imports & dunder attrs are r
- Language: py
- Symbols:
...
## eval/smoke.py
- Layer: utility
- Doc: Smoke test: load the TopoGPT3 checkpoint and produce a small completion.  Used as the first gate: if this fails we abort
- Language: py
- Symbols:
...
## eval/temp_sweep.py
- Layer: utility
- Doc: Barrido de temperatura x top-k sobre HumanEval.  Mide pass@1 en modo greedy (T=0) y pass@5 a temperaturas crecientes par
- Language: py
- Symbols:
